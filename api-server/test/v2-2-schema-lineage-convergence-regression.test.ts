...
test("historical database lineages converge before V2 location columns", async () => {
  ...
  assert.match(auth, /publicId: publicUserId\(normalizedRole, newUserId\)/);
  assert.match(admin, /publicId: publicUserId\("admin", newAdminId\)/);
  assert.match(chat, /const pairKey = chatPairKey\(userId, otherUserId\)/);
});
