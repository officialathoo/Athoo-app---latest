...
// Thrown when the selected storage provider is missing required
// configuration (env vars). Never includes secret values -- only which
// setting is absent -- so it is safe to surface in logs and sanitized API
// error responses.
...
export class R2StorageProvider implements StorageProvider {
  ...
  constructor() {
    ...
    if (!endpoint || !accessKeyId || !secretAccessKey || !this.bucket) {
      ...
      if (!endpoint) missing.push("S3_ENDPOINT (or CLOUDFLARE_R2_ACCOUNT_ID)");
      if (!accessKeyId) missing.push("CLOUDFLARE_R2_ACCESS_KEY_ID (or S3_ACCESS_KEY_ID)");
      if (!secretAccessKey) missing.push("CLOUDFLARE_R2_SECRET_ACCESS_KEY (or S3_SECRET_ACCESS_KEY)");
      if (!this.bucket) missing.push("CLOUDFLARE_R2_BUCKET (or S3_BUCKET)");
      throw new StorageNotConfiguredError(
        `Storage provider 'r2' is missing required configuration: ${missing.join(", ")}.`
      );
    }
