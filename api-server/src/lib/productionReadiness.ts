...
export function productionReadinessSnapshot() {
  ...
  if (!process.env.TURN_URL) issues.push({ area: "calls", severity: "high", message: "TURN server is not configured", fix: "Set TURN_URL, TURN_USERNAME, TURN_CREDENTIAL for reliable calls." });
  if (!process.env.GOOGLE_MAPS_API_KEY) issues.push({ area: "maps", severity: "high", message: "Google Maps API key missing", fix: "Enable Maps, Geocoding, Places, Directions APIs and set GOOGLE_MAPS_API_KEY." });
