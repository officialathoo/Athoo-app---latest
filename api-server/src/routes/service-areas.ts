...
// Canonical Pakistan provinces / territories — single source of truth for spelling.
// Used to validate the `province` field on every service area so admin data never drifts
// (e.g. "ICT" vs "Islamabad Capital Territory", "KPK" vs "Khyber Pakhtunkhwa").
...
// Genuinely Pakistan-wide default city list (name, province) spanning every province/territory.
// Used only to seed a brand-new/empty service_areas table — admins can add/edit/deactivate afterward.
export const PAKISTAN_WIDE_DEFAULT_AREAS: Array<[string, string]> = [
  ...
  ["Kasur", "Punjab"], ["Okara", "Punjab"], ["Dera Ghazi Khan", "Punjab"], ["Rahim Yar Khan", "Punjab"],
  ["Attock", "Punjab"], ["Wah Cantonment", "Punjab"], ["Chiniot", "Punjab"], ["Vehari", "Punjab"],
  ["Karachi", "Sindh"], ["Hyderabad", "Sindh"], ["Sukkur", "Sindh"], ["Larkana", "Sindh"],
  ["Nawabshah", "Sindh"], ["Mirpur Khas", "Sindh"], ["Jacobabad", "Sindh"], ["Shikarpur", "Sindh"],
  ["Peshawar", "Khyber Pakhtunkhwa"], ["Mardan", "Khyber Pakhtunkhwa"], ["Abbottabad", "Khyber Pakhtunkhwa"],
  ["Swat", "Khyber Pakhtunkhwa"], ["Kohat", "Khyber Pakhtunkhwa"], ["Bannu", "Khyber Pakhtunkhwa"],
  ["Dera Ismail Khan", "Khyber Pakhtunkhwa"], ["Nowshera", "Khyber Pakhtunkhwa"],
  ["Quetta", "Balochistan"], ["Gwadar", "Balochistan"], ["Sibi", "Balochistan"], ["Turbat", "Balochistan"],
  ["Khuzdar", "Balochistan"], ["Chaman", "Balochistan"],
  ["Islamabad", "Islamabad Capital Territory"],
  ["Muzaffarabad", "Azad Jammu & Kashmir"], ["Mirpur", "Azad Jammu & Kashmir"], ["Rawalakot", "Azad Jammu & Kashmir"],
...
serviceAreasAdminRouter.post("/", requirePermission("settings.write"), async (req: AuthRequest, res) => {
  try {
    ...
    if (province && !(PAKISTAN_PROVINCES as readonly string[]).includes(province)) {
      return res.status(400).json({ error: `Invalid province. Must be one of: ${PAKISTAN_PROVINCES.join(", ")}` });
    }
    ...
    const duplicate = await db.query.serviceAreasTable.findFirst({ where: and(
      sql`lower(trim(${serviceAreasTable.name})) = lower(trim(${name}))`,
      province
        ? sql`lower(trim(${serviceAreasTable.province})) = lower(trim(${province}))`
        : isNull(serviceAreasTable.province),
    ) });
...
serviceAreasAdminRouter.patch("/:id", requirePermission("settings.write"), async (req: AuthRequest, res) => {
  try {
    ...
    if (nextProvince && !(PAKISTAN_PROVINCES as readonly string[]).includes(nextProvince)) {
      return res.status(400).json({ error: `Invalid province. Must be one of: ${PAKISTAN_PROVINCES.join(", ")}` });
    }
    const duplicate = await db.query.serviceAreasTable.findFirst({ where: and(
      sql`lower(trim(${serviceAreasTable.name})) = lower(trim(${nextName}))`,
      nextProvince
        ? sql`lower(trim(${serviceAreasTable.province})) = lower(trim(${nextProvince}))`
        : isNull(serviceAreasTable.province),
...
export async function seedServiceAreasIfEmpty() {
  try {
    ...
    await db.insert(serviceAreasTable).values(PAKISTAN_WIDE_DEFAULT_AREAS.map(([name, province], sortOrder) => ({ id: genId(), name, province, sortOrder, isActive: true, createdAt: new Date(), updatedAt: new Date() })));
