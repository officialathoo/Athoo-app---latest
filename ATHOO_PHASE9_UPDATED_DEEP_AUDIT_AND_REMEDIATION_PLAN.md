...
## Phase 8 completion — Governance, inactivity lifecycle and accessibility

Status: source remediation complete against `ATHOO_PHASE7_SESSION_SECURITY_FIXED.zip`.

Completed:
1. Added staged, configurable inactivity warning/restriction/review automation with rate-limited activity tracking.
2. Added safe provider matching pause and explicit return-to-availability behavior.
3. Added an audited admin inactivity review queue, exact notification routing, manual reminders, review clearing, and manual deactivation.
4. Guaranteed that inactivity never automatically permanently deletes an account.
5. Added concurrency-safe lifecycle sweeps and active-account guards on inactivity-specific actions.
6. Added versioned bilingual policy governance with public mobile consumption, permissioned admin editing, draft review, publication, audit trails, and acceptance-version safety.
7. Added nine seeded business policy documents and mobile offline/fallback policy access.
8. Added admin skip navigation, focus visibility, reduced-motion handling, labelled navigation state, and responsive governance/settings layouts.
9. Confirmed shared iOS/Android splash configuration, no audited hardcoded black screen background, and retained invite-card margins.
10. Added `20260716_workflow_inactivity_policy_governance.sql` as the authoritative latest migration.

Validation: 9/9 targeted Phase 8 tests, 425/425 complete regression tests, and 31/31 changed TypeScript/TSX syntax checks passed. Project, security, React Native style, Expo workspace, mobile release, and release configuration checks passed.

## Final phase — Connected production certification and release decision

- Install dependencies and run complete workspace typecheck, tests, and production builds.
- Back up Neon, apply every pending migration, and run status/verification/integrity checks.
- Deploy API/admin and verify Render/Vercel health and CORS.
- Verify R2 upload, authenticated viewing, retention, replacement, and deletion workflows.
- Create fresh Android and iOS builds for notification channels, sounds, biometrics, and WebRTC.
- Execute the full customer/provider/admin matrix: registration, one-device sessions, profile/rates/categories, broadcast matching, notifications, chat, negotiation, bookings, finance, support, premium, restrictions, deletion, policies, and admin destinations.
- Verify voice calling with TURN on Wi-Fi/mobile-data combinations.
- Obtain legal review of public policies before launch.
- Record evidence, unresolved risks, rollback readiness, and an explicit release/no-release decision.

## Phase 9 completion — Production certification preparation

Phase 9 closed the final source-side certification gaps. TURN readiness is now a production environment and deep-health gate rather than only a warning. The Render blueprint exposes the required provider-neutral TURN, session, lifecycle, monitoring, and escalation inputs. Connected verification now requires a controlled provider and checks provider broadcast eligibility, policies, admin operational queues, and production call readiness.

Device evidence and the final release decision now explicitly cover every originally reported critical problem: customer-to-provider broadcasts, chat keyboard and unread badges, message notifications and sounds, provider rates and multiple categories, two-way calls, one-device biometrics, admin exact destinations, inactivity safety, policies, legal review, secret rotation, monitoring, and hosting capacity.

Validation: 19/19 targeted checks and 431/431 complete regression tests passed. Project, release, operations, blueprint, security, Expo workspace, React Native style, mobile release, beta, device-preparation, syntax, and synthetic production-environment checks passed.

Status: source certification preparation complete. Connected production evidence remains mandatory. The current decision is `CONDITIONAL-NO-GO` until Neon, Render, Vercel, R2, EAS, TURN, Android, iPhone, legal, monitoring, and operational approval evidence is complete.
