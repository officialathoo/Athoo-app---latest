...
function walk(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (["node_modules", ".git", "dist", ".expo", ".cache", "coverage", "build"].includes(entry.name)) continue;
...
function findNamed(dir, names) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (["node_modules", ".git", ".cache", "dist", "build", "coverage"].includes(entry.name)) continue;
    const full = path.join(dir, entry.name);
