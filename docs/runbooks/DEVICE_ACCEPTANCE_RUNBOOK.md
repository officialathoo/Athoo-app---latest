...
This runbook validates the exact `ATHOO_V2_2_PRODUCTION_CERTIFICATION.zip` candidate on one physical Android phone and one physical iPhone. Do not reuse evidence from an older ZIP, commit, EAS build, Render deploy or Vercel deploy.
...
Record each EAS build ID, artifact URL, creation time, source commit, runtime version, update channel and **EAS Update group**. Android and iOS evidence must identify the exact update group installed on each device so OTA evidence cannot be reused across releases. Use dedicated customer, provider and administrator acceptance accounts. Do not use personal production accounts.
...
2. Open every map screen and confirm streets/tiles render behind the marker; a gray or white surface with only a red dot or attribution fails. Pan and zoom to force additional tile requests.
...
3. Prove uninterrupted two-way voice in both directions for at least 60 seconds.
4. Record **Cloudflare TURN relay verified** only after confirming that the selected WebRTC candidate is an authenticated Cloudflare TURN relay. A direct-only connection, fallback-audio path, one-way audio or missing relay evidence fails production acceptance.
...
3. Change search, type and date filters rapidly and confirm stale responses do not overwrite the latest results.
...
6. Verify upload-scanner status, maintenance counters and safe scanner test actions in the admin panel.

## 5. Cross-role setup

Use the Android and iPhone simultaneously for booking, negotiation, single-accept broadcast, chat, secure media, arrival, PIN start/completion, scheduled-job reminders, refund evidence/status, signed invoice verification, live location, notifications, session replacement and calls. Use the admin panel in a separate browser for approvals, exact notification routing, account restriction and availability override.
