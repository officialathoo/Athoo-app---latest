...
Athoo is structured to be provider-agnostic: backend API, admin panel, PostgreSQL durable queue, cache policy, storage, maps, notifications, email, and calling infrastructure are configured through deployment settings.
...
3. Set `API_BASE_URL`, `ADMIN_BASE_URL`, `CORS_ORIGINS`, `DATABASE_URL`, queue, cache, storage, notification, email, map, and TURN variables. Use `CACHE_PROVIDER=memory` only with one API instance; Redis is not enabled until the shared adapter migration is complete.
