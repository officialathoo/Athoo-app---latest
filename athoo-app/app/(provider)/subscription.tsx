...
export default function SubscriptionScreen() {
  ...
  async function pickScreenshot() {
    ...
    try {
      ...
      showError("Upload Failed", e?.message || "Could not upload screenshot. Please try again.");
    } finally {
  ...
  async function handleSubscribe() {
    ...
    if (!screenshot) {
      Alert.alert("Required", "Please upload a screenshot of your payment as proof.");
      return;
    }
    if (!requestIdRef.current) requestIdRef.current = `subscription-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
    setSubscribing(true);
    try {
      await api.subscribeToPlan({ planId: selectedPlan.id, billingPeriod: billing, paymentReference: payRef.trim(), screenshotUrl: screenshot, clientRequestId: requestIdRef.current });
  ...
  const STATUS_COLORS: Record<string, string> = {
    ...
    cancellation_scheduled: Colors.warning,
  };
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <View style={styles.modalHeader}>
              ...
              <Pressable onPress={() => { setShowModal(false); setPayRef(""); setScreenshot(null); }} style={styles.modalCloseBtn}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.modalBody}>
                ...
                <Pressable style={styles.photoBtn} onPress={pickScreenshot} disabled={uploadingScreenshot}>
                  {uploadingScreenshot ? (
                    ...
                    <View style={styles.photoPreviewRow}>
                      ...
                      <Pressable style={styles.photoRemove} onPress={() => setScreenshot(null)}>
                        ...
                        <Text style={styles.photoRemoveText}>Remove & replace</Text>
                      </Pressable>
                    </View>
                  ) : (
                    <View style={styles.photoBtnInner}>
                ...
                <Pressable
                  style={[styles.submitBtn, (!payRef.trim() || !screenshot || subscribing || uploadingScreenshot) && styles.btnDisabled]}
                  onPress={handleSubscribe}
                  disabled={!payRef.trim() || !screenshot || subscribing || uploadingScreenshot}
                >
                  {subscribing ? (
...
const styles = StyleSheet.create({
  ...
  cancelScheduledTitle: { fontSize: 14, fontWeight: "800", color: Colors.warning },
  cancelScheduledSub: { fontSize: 12, color: Colors.textSecondary, lineHeight: 18, marginTop: 2 },
