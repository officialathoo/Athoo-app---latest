...
export type SubscriptionAudience = "customer" | "provider";
...
export function CustomerSubscriptionScreen() {
  const { theme } = useTheme();
  const { isUrdu, formatCurrency, formatDate: formatLocalizedDate, translate: tr } = useLang();
  const styles = useMemo(() => createCustomerSubscriptionStyles(theme, isUrdu), [theme, isUrdu]);
  ...
  return (
    <View style={[styles.container, { paddingTop: topPad }]}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>{tr("Athoo Premium")}</Text>
          <Text style={styles.headerSub}>{tr("Get more out of every booking")}</Text>
        </View>
      </View>
            ...
            <AnimatedCard delay={80}>
              <View style={styles.activeBanner}>
                ...
                <Pressable style={styles.cancelBtn} onPress={handleCancel} disabled={cancelling}>
                  {cancelling ? <ActivityIndicator size="small" color={theme.colors.danger} /> : <Text style={styles.cancelBtnText}>{tr("Cancel")}</Text>}
                </Pressable>
              </View>
            </AnimatedCard>
          )}
            ...
            <View style={styles.billingToggle}>
              ...
              <Pressable onPress={() => setBilling("yearly")} style={[styles.billingBtn, billing === "yearly" && styles.billingBtnActive]}>
                <Text style={[styles.billingText, billing === "yearly" && styles.billingTextActive]}>
                  {tr("Yearly")} <Text style={styles.saveBadge}> · {tr("20% off")}</Text>
                </Text>
              </Pressable>
            </View>
          </AnimatedCard>
          ...
          {plans.length === 0 ? (
            <AnimatedCard delay={140}>
              <View style={styles.emptyBox}>
                ...
                <Text style={styles.emptyTitle}>{tr("Premium plans coming soon!")}</Text>
                <Text style={styles.emptyText}>{tr("Stay tuned for exclusive member benefits.")}</Text>
              </View>
            </AnimatedCard>
          ) : (
            plans.map((plan, i) => {
          ...
          <AnimatedCard delay={380}>
            <View style={styles.noteBox}>
              ...
              <Text style={styles.noteText}>
                {tr("After subscribing, send payment through the available payment method and enter the transaction ID. Athoo will notify you after review.")}
              </Text>
            </View>
          </AnimatedCard>
        </ScrollView>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{tr("Subscribe")} — {selectedPlan?.name}</Text>
...
export function ProviderSubscriptionScreen() {
  const { theme } = useTheme();
  const { isUrdu, formatCurrency, formatDate: formatLocalizedDate, translate: tr } = useLang();
  const styles = useMemo(() => createProviderSubscriptionStyles(theme, isUrdu), [theme, isUrdu]);
  const insets = useSafeAreaInsets();
  const topPad = Platform.OS === "web" ? 67 : insets.top;
  const { user } = useAuth();
  const { showSuccess, showError } = useToast();
...
export default function SubscriptionScreen({ audience = "customer" }: { audience?: SubscriptionAudience }) {
  return audience === "provider" ? <ProviderSubscriptionScreen /> : <CustomerSubscriptionScreen />;
}

function createCustomerSubscriptionStyles(theme: AthooTheme, isUrdu: boolean) {
  return StyleSheet.create({
    ...
    heroPerkText: { fontSize: 13, color: theme.colors.white, fontWeight: "600" },

    activeBanner: {
      flexDirection: isUrdu ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between",
      backgroundColor: theme.colors.success + "12", borderRadius: 14, padding: 14,
      borderWidth: 1, borderColor: theme.colors.success + "30",
    },
    activeBannerLeft: { flexDirection: isUrdu ? "row-reverse" : "row", alignItems: "center", gap: 10, flex: 1 },
    activeBannerTitle: { fontSize: 14, fontWeight: "800", color: theme.colors.success },
    activeBannerSub: { fontSize: 12, color: theme.colors.textSecondary, marginTop: 2 },
    cancelBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10, borderWidth: 1, borderColor: theme.colors.danger + "40" },
    cancelBtnText: { fontSize: 12, fontWeight: "700", color: theme.colors.danger },
...
function createProviderSubscriptionStyles(theme: AthooTheme, isUrdu: boolean) {
  return StyleSheet.create({
    container: { flex: 1, backgroundColor: theme.colors.background },
    center: { flex: 1, alignItems: "center", justifyContent: "center" },
    scroll: {
    ...
    historyAmt: { fontSize: 13, fontWeight: "700", color: theme.colors.text },
    historyDate: { fontSize: 12, color: theme.colors.textMuted, marginTop: 2 },
    historyStatus: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: radius.pill },
    historyStatusText: { fontSize: 11, fontWeight: "700" },
